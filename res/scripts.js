...
// 08. Sticky Navigation Scroll Update
// ----------------------------------------
let mobileStickyNavTrigger = null; // Track the current ScrollTrigger instance
let isMobileStickyEnabled = false;

function createMobileStickyNav() {
    const isMobile = window.innerWidth < 768;

    // Remove if it exists and no longer needed
    if (mobileStickyNavTrigger && !isMobile) {
        mobileStickyNavTrigger.kill();
        mobileStickyNavTrigger = null;
        isMobileStickyEnabled = false;
        gsap.set(".navigation", { clearProps: "y" }); // Reset position
        return;
    }

    // Create if it doesn't exist and is needed
    if (isMobile && !isMobileStickyEnabled) {
        ...
        isMobileStickyEnabled = true;
    }
}
// Initialize on load
createMobileStickyNav();
